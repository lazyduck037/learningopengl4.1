...
// g++ -Wall -pedantic -o skin.exe main.cpp gl_utils.cpp maths_funcs.cpp -I ../common/include ../common/win32/libglew32.dll.a ../common/win32/glfw3dll.a ../common/win32/assimp.lib -lOpenGL32 -L ./ -lglew32 -lglfw3 -lm
...
#define ONE_DEG_IN_RAD ( 2.0 * M_PI ) / 360.0 // 0.017444444
	...
	for ( int i = 0; i < MAX_BONES; i++ ) {
		...
		printf("name:%s\n",name);
		printf("bone_matrices_locations[i] :%d\n",bone_matrices_locations[i]);
	...
	while ( !glfwWindowShouldClose( g_window ) ) {
		...
		if ( monkey_moved ) {
			...
			glUniformMatrix4fv( bone_matrices_locations[0], 1, GL_FALSE,
													monkey_bone_animation_mats[0].m );
			glUniformMatrix4fv( bone_matrices_locations[1], 1, GL_FALSE,
													monkey_bone_animation_mats[1].m );
			glUniformMatrix4fv( bone_matrices_locations[2], 1, GL_FALSE,
													monkey_bone_animation_mats[2].m );
