...
//linux : g++ -Wall -pedantic -o lesson-5 main.cpp gl_utils.cpp maths_funcs.cpp obj_parser.cpp -I ../common/include/ ../common/linux_x86_64/libGLEW.a -lglfw -lGL
//define const
...
mat4 defineViewMax()
{
	...
	mat4 view_mat_temp = R * T;
    return view_mat_temp;
}

void draw(int model_mat_location,mat4 model_mats, int point_count)
{
    glUniformMatrix4fv( model_mat_location, 1, GL_FALSE, model_mats.m );
    glDrawArrays( GL_TRIANGLES, 0, point_count );
}

void controlKeyboard(int view_mat_location,double elapsed_seconds)
{
...
int main() {
    ...
    mat4 proj_mat = defineProjectMax();
	view_mat = defineViewMax();
    glUseProgram( shader_programme );
	glUniformMatrix4fv( view_mat_location, 1, GL_FALSE, view_mat.m );
	glUniformMatrix4fv( proj_mat_location, 1, GL_FALSE, proj_mat.m );

    mat4 model_mats;
    model_mats = translate( identity_mat4(), sphere_pos_wor[0] );
    ...
    while ( !glfwWindowShouldClose( g_window ) ) 
    {
        ...
        draw(model_mat_location, model_mats, point_count); 
        glfwPollEvents();

        controlKeyboard(view_mat_location,elapsed_seconds);
